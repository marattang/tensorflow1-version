...
# 모델구성
# layer 1
w1 = tf.get_variable('w1', shape=[2, 2, 1, 128]) #(kernel_size, input(마지막 배열), output)
...
# layer 2
w2 = tf.get_variable('w2', shape=[2, 2, 128, 128])
...
w3 = tf.get_variable('w3', shape=[2, 2, 128, 64])
...
# layer 5 DNN
w5 = tf.get_variable('w5', shape=[2*2*64, 256])
b5 = tf.Variable(tf.random_normal([256]), name='b1')
L5 = tf.matmul(L_flat, w5) + b5
L5 = tf.nn.sigmoid(L5)
print(L5) # (?, 64)

# layer 6 DNN
w6 = tf.get_variable('w6', shape=[256, 128])
b6 = tf.Variable(tf.random_normal([128]), name='b2')
